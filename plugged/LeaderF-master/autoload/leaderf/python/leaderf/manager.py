...
class Manager(object):
    ...
    def _pageUpInPopup(self):
        if self.isPreviewWindowOpen():
            scroll_step_size = int(lfEval("get(g:, 'Lf_PreviewScrollStepSize', 1)"))
            if lfEval("has('nvim')") == '1':
                cur_winid = lfEval("win_getid()")
                lfCmd("noautocmd call win_gotoid(%d)" % self._preview_winid)
                lfCmd("norm! H") # 光标定位到页面顶部
                lfCmd("norm! zb") # 当前光标所在行移到页面底部
                lfCmd("redraw")
                lfCmd("noautocmd call win_gotoid(%s)" % cur_winid)
            else:
                lfCmd("call win_execute(%d, 'norm! %dj')" % (self._preview_winid, scroll_step_size))

    def _pageDownInPopup(self):
        if self.isPreviewWindowOpen():
            scroll_step_size = int(lfEval("get(g:, 'Lf_PreviewScrollStepSize', 1)"))
            if lfEval("has('nvim')") == '1':
                cur_winid = lfEval("win_getid()")
                lfCmd("noautocmd call win_gotoid(%d)" % self._preview_winid)
                lfCmd("norm! L") # 光标定位到页面底部
                lfCmd("norm! zt") # 当前光标所在行移到页面顶部
                lfCmd("redraw")
                lfCmd("noautocmd call win_gotoid(%s)" % cur_winid)
            else:
                lfCmd("call win_execute(%d, 'norm! %dj')" % (self._preview_winid, scroll_step_size))

    def _toPrevFuncInPopup(self):
        if self.isPreviewWindowOpen():
            scroll_step_size = int(lfEval("get(g:, 'Lf_PreviewScrollStepSize', 1)"))
            if lfEval("has('nvim')") == '1':
                cur_winid = lfEval("win_getid()")
                lfCmd("noautocmd call win_gotoid(%d)" % self._preview_winid)
                # lfCmd("call search('\(\(if\|for\|while\|switch\|catch\)\_s*\)\@64<!(\_[^)]*)\_[^;{}()]*\zs{', 'bw')") # 查找前一个函数
                lfCmd("call search('^\s*\(\i\+\_[ \t\*]\+\)\+\i\+\_s*(\_[^)]*)\_s*{', 'bw')") # 查找前一个函数
                lfCmd("redraw")
                lfCmd("noautocmd call win_gotoid(%s)" % cur_winid)
            else:
                lfCmd("call win_execute(%d, 'norm! %dj')" % (self._preview_winid, scroll_step_size))

    def _toNextFuncInPopup(self):
        if self.isPreviewWindowOpen():
            scroll_step_size = int(lfEval("get(g:, 'Lf_PreviewScrollStepSize', 1)"))
            if lfEval("has('nvim')") == '1':
                cur_winid = lfEval("win_getid()")
                lfCmd("noautocmd call win_gotoid(%d)" % self._preview_winid)
                # lfCmd("call search('\(\(if\|for\|while\|switch\|catch\)\_s*\)\@64<!(\_[^)]*)\_[^;{}()]*\zs{', 'w')") # 查找后一个函数
                lfCmd(r"call search('^\s*\(\i\+\_[ \t\*]\+\)\+\i\+\_s*(\_[^)]*)\_s*{', 'w')") # 查找后一个函数
                lfCmd("redraw")
                lfCmd("noautocmd call win_gotoid(%s)" % cur_winid)
            else:
                lfCmd("call win_execute(%d, 'norm! %dj')" % (self._preview_winid, scroll_step_size))
    ...
    def input(self):
        ...
        for cmd in self._cli.input(self._callback):
            ...
            if equal(cmd, '<Update>'):
                ...
                self._pageDownInPopup()
            elif equal(cmd, '<C-Left>'):
                self._toPrevFuncInPopup()
            elif equal(cmd, '<C-Right>'):
                self._toNextFuncInPopup()
