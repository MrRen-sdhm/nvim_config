	...
	if name != ''
		...
		elseif s:snip_engine == 1
			if mode(1) =~ 'i'
				...
				call feedkeys(name . "\<c-r>=UltiSnips#ExpandSnippet()\<cr>", 'n')
				" unsilent echom "col: ". col('.')
			else
				" call feedkeys('o' . name . "\<c-r>=UltiSnips#ExpandSnippet()\<cr>", '!')
				call feedkeys('o' . name . "\<c-r>=UltiSnips#ExpandSnippet()\<cr>", 'n')
			endif
		endif
	endif
endfunc
